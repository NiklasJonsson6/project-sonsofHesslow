...
public class FilledBeizierPath {
    ...
    //todo real precision & concave shapes.
    public FilledBeizierPath(Vector2[] points,float[] matrix) //alternating. start ctl ctl point ctl ctl point ctl ctl (start)
    {
        ...
        Vector2[] segmented = new Vector2[(int)precision*(points.length)/3];
        for(int j = 0;j<points.length/3;j++)
        {
            ...
            {
                segmented[j*(int)precision+i] = beiz_4(start,control_1,control_2,end,i/(precision));
            }
        }

        float[] color = {0.2f,0.7f,0.9f,1f};

        // Daniel: Should this not be -2?? are we missing drawing some one?
        // this get the correct result (I think) but it feels wrong. Investigate!
        short[] tris = new short[(segmented.length-1)*3];
        //wire both ways if that becomes necessary.
        for(int i = 0; i< segmented.length-1;i++)
        {
            if(i%2==0)
            {
                tris[i*3+0] = ((short)i);
        ...
        m = new Mesh(tris, segmented, color, matrix);
    }
